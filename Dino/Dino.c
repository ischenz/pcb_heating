...
void introMessage() {
  DINO_OLED_NewFrame();
  DINO_OLED_PrintString((128 - 80) / 2, 8, "小恐龙游戏", &font16x16, DINO_OLED_COLOR_NORMAL);
  DINO_OLED_DrawImage(DINO_INIT_X, DINO_INIT_Y, &dino_jumpsImg, DINO_OLED_COLOR_NORMAL);
  DINO_OLED_PrintString((128 - 32) / 2, 43, "开始", &font16x16, DINO_OLED_COLOR_NORMAL);
  DINO_OLED_ShowFrame();
}
...
void showLine() {
  ...
  DINO_OLED_DrawImage(DINO_INIT_X, DINO_INIT_Y, &dino_jumpsImg, DINO_OLED_COLOR_NORMAL);
  for (;;) {
    DINO_OLED_DrawLine(BASE_LINE_X, BASE_LINE_Y, line_end, BASE_LINE_Y, DINO_OLED_COLOR_NORMAL);
    DINO_OLED_ShowFrame();
    line_end += 4;
...
void moveDino(int16_t y, int type) {
  if (type == 0)
    DINO_OLED_DrawImage(DINO_INIT_X, y, &dino_front_legImg, DINO_OLED_COLOR_NORMAL);
  else if (type == 1)
    DINO_OLED_DrawImage(DINO_INIT_X, y, &dino_back_legImg, DINO_OLED_COLOR_NORMAL);
  else if (type == -1)
    DINO_OLED_DrawImage(DINO_INIT_X, y, &dino_jumpsImg, DINO_OLED_COLOR_NORMAL);
  else if (type == -2)
    DINO_OLED_DrawImage(DINO_INIT_X, y, &dino_crashedImg, DINO_OLED_COLOR_NORMAL);
}
...
void moveTree(int16_t x, Image *type) {
  ...
  DINO_OLED_DrawImage(x, BASE_LINE_Y - type->h, type, DINO_OLED_COLOR_NORMAL);
}
...
void displayScore(int score, int8_t type) {
  ...
  if (type == 0) {
    sprintf(tmp, "%d", score);
    DINO_OLED_PrintString(72, 3, "得分：", &font12x12, DINO_OLED_COLOR_NORMAL);
    DINO_OLED_PrintString(72 + 30, 3, tmp, &font12x12, DINO_OLED_COLOR_NORMAL);
  }

  else {
    sprintf(tmp, "%d", highest_score);
    DINO_OLED_PrintString(72 - 12, 22, "最高分：", &font12x12, DINO_OLED_COLOR_NORMAL);
    DINO_OLED_PrintString(72 + 30, 22, tmp, &font12x12, DINO_OLED_COLOR_NORMAL);
  }
}
...
uint8_t play() {
  ...
  for (;;) {
    ...
    if (tree_x <= (dino_right - ((jump < 5) ? jump : 5)) && tree_x > (DINO_INIT_X + 1) && (dino_y + dino_back_legImg.h) >= (BASE_LINE_Y - tree->h)) {
      ...
      DINO_OLED_DrawLine(BASE_LINE_X, BASE_LINE_Y, BASE_LINE_X1, BASE_LINE_Y, DINO_OLED_COLOR_NORMAL);
      displayScore(score, 0);
      if (score > highest_score) {
        highest_score = score;
        DINO_OLED_PrintString(75, 22, "新纪录！", &font12x12, DINO_OLED_COLOR_NORMAL);
      } else {
