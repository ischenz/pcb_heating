...
//void Send_Byte(uint8_t dat)
//{
//    HAL_I2C_Master_Transmit(&hi2c1, 0x78, &dat, 1, 100);
//}
...
void OLED_WR_Byte(uint8_t dat,uint8_t mode)
{
    ...
    HAL_I2C_Master_Transmit(&hi2c1, 0x78, data, 2, 500);
}
...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
        ...
        for(n=0;n<128;n++)
		{
            HAL_I2C_Mem_Write(&hi2c1, 0x78, 0x40, 1, &OLED_GRAM[n][i], 1, 500);
        }
  }
}
//清屏函数
...
//size:字体大小
//mode:0,反色显示;1,正常显示
void OLED_ShowFNum(uint8_t x,uint8_t y,float num,uint8_t len,uint8_t size1,uint8_t mode)
{
